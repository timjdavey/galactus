...
TIGHT = {"Inc": 1.3, "D": 3, "Ydisk": 15, "Ybul": 15}
    ...
    with pm.Model(coords=coords) as galaxy_model:
        ...
        if train_y:
            SurfaceNormal = pm.Bound(
                pm.Normal, lower=0.2, upper=1.5
            )  # reasonable physical bounds
            Ydisk = SurfaceNormal(
                "Ydisk",
                mu=reference.Ydisk,
                sigma=reference.e_Ydisk / TIGHT["Ydisk"],
                dims="Galaxy",
            )
            Ybul = SurfaceNormal(
                "Ybul",
                mu=reference.Ybul,
                sigma=reference.e_Ybul / TIGHT["Ybul"],
                dims="Galaxy",
            )
        ...
        # Define likelihood
        obs = pm.Normal(
            "obs", mu=Vpred, sigma=df.e_Vobs, observed=df.Vobs, dims="Observation"
        )
