...
class Simulation:
    ...
    """
    def __init__(self, masses, space, G=1, lazy=False, cp=None):
    ...
    def analyse(self, sub_list=None):
        ...
        fields = dict([(d, []) for d in space.dimensions])
        fields['abs'] = []
        ...
        for i in range(total_mass_points):
            ...
            self.log("%s of %s" % (i,total_mass_points))
            M = mass_sum[mass_points]
            
            # fields for each mass point
            mass_distances = np.array(mass_points)*space.scale
            point_fields = dict([(d, blank.copy()) for d in space.dimensions])
            point_fields['abs'] = blank.copy()
            ...
            for observed_points in point_list:
                ...
                # so leave as 0
                    ...
                    deltas = observed_points*space.scale-mass_distances
                    ...
                    for i, delta in enumerate(deltas):
                        point_fields[i][ijk] = -F*delta/r
        
                    point_fields['abs'][ijk] = np.abs(F)
                
                    for d, field in point_fields.items():
                        fields[d].append(field)
    
        self.log('Completed %s' % total_mass_points)
        
        self.field_components = fields

        for dim, field in fields.items():
            sums[dim] = np.sum(field, axis=0)
    ...
    def heatmaps(self, slices=None, image_scale=15):
        ...
        fig, axes = plt.subplots(how_many, 5,
            figsize=(image_scale,how_many*image_scale/5))
    ...
    def mass_profile(self, stack=None):
        ...
        sns.heatmap(self.sums['mass'][stack], xticklabels=self.space.x, yticklabels=self.space.coords[1]*self.space.scale)
